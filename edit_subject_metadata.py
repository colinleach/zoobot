...
def edit_subjects_with_api(subject_set_id, login_loc, func_to_apply):
    ...
    if subject_set_id == '74807':  # priority
        ...
        print('subject set not recognised')
        total = None
    ...
    with tqdm(total=total) as pbar:  # or 6000 in random set
        while True:
            ...
            subject = func_to_apply(subject)
            subject.save()
            pbar.update()


def edit_subjects_in_files(locs, func_to_apply):
    # with tqdm() as pbar:
    for loc_n, loc in enumerate(locs):
        print('Modifying {} ({} of {})'.format(loc, loc_n, len(locs)))
        with open(loc, 'r') as f_read: 
            with open(loc.split('.')[-2] + '_updated.' + loc.split('.')[-1], 'w') as f_write:
                while True:
                    ...
                    if line == '':
                        ...
                        classification['links']['subject'] = func_to_apply(classification['links']['subject'])
                        f_write.write(json.dumps(classification) + '\n')  # newline is crucial
                            # pbar.update()


def remove_classification_metadata(subject):
...
# TODO run this on raw download as well
def fix_keys(subject):
    mistaken_keys = set(['iauname', 'IAUNAME', '!IAUNAME'])
    desired_key = '!iauname'
    for included_mistaken_key in mistaken_keys.intersection(subject['metadata'].keys()):
        if desired_key not in subject['metadata'].keys():
            subject['metadata'][desired_key] = subject['metadata'][included_mistaken_key]
            print('Edited {}:{} to {}:{}'.format(
                included_mistaken_key,
                subject['metadata'][included_mistaken_key], 
                desired_key, 
                subject['metadata'][desired_key]
                )
            )
            del subject['metadata'][included_mistaken_key]
        else:
            raise KeyError('Conflict: use {}:{} or {}:{}?'.format(
                included_mistaken_key, 
                subject['metadata'][included_mistaken_key],
                desired_key,
                subject['metadata'][desired_key]
...
if __name__ == '__main__':
    ...
    # edit_subjects_with_api(subject_set_id, login_loc, fix_keys)

    # input_dir = 'data/decals/classifications/streaming/raw'
    # locs = get_chunk_files(input_dir, derived=False)
    locs = ['data/decals/classifications/streaming/raw_hidden/panoptes_api_first_117640603_last_156463580_bad_iauname.json']
    # locs = ['data/decals/classifications/streaming/raw_hidden/panoptes_api_first_0_last_117640575_bad_iauname.json']
    edit_subjects_in_files(locs, fix_keys)
