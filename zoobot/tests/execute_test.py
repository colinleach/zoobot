...
from zoobot.active_learning import setup, active_learning, make_shards, execute
...
@pytest.fixture()
def active_config(shard_config, tmpdir):
    config = execute.ActiveConfig(shard_config, run_dir=tmpdir.mkdir('run_dir').strpath)
    config.iterations = 2
    return config
...
def test_run(active_config_ready, tmpdir, monkeypatch, catalog_random_images, tfrecord_dir, acquisition_func):
    ...
    setup.make_database_and_shards(catalog, active_config.db_loc, active_config.shards.initial_size, tfrecord_dir, shard_size=25)
    ...
    active_config.run(catalog, train_callable, get_acquistion_func)
