...
def create_decals_master_catalog(catalog_loc: str, classifications_loc: str, save_loc: str):
    ...
    catalog['png_loc'] = catalog['local_png_loc'].apply(
...
def specify_file_locs(df):
    ...
    if root_loc is not 'local':
        # + sign to allow pandas to override
        df['file_loc'] = root_loc + '/' + df['png_loc']
    else:
