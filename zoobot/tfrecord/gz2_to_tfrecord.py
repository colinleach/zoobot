...
def write_catalog_to_train_test_tfrecords(df, label_col, train_loc, test_loc, img_size, columns_to_save, train_test_fraction=0.8):
    ...
    write_image_df_to_tfrecord(train_df, label_col, train_loc, img_size, columns_to_save)
    write_image_df_to_tfrecord(test_df, label_col, test_loc, img_size, columns_to_save)

    return train_df, test_df


def write_image_df_to_tfrecord(df, label_col, tfrecord_loc, img_size, columns_to_save, append=False):
    ...
    for subject_n, subject in tqdm(df.iterrows(), total=len(df), unit=' subjects saved'):
        if subject['png_ready']:
            ...
            img.thumbnail(dimensions)  # inplace on img  # TODO split into fits/png. If fits, rescale directly
            matrix = np.array(img)
...
if __name__ == '__main__':
    ...
    for size in [128, 256, 512]:
        ...
        write_catalog_to_train_test_tfrecords(df, label_col, train_loc, test_loc, size, columns_to_save)
