...
def custom_top_beta(model, output_dim, schema, batch_size):
    model.add(tf.keras.layers.Dense(output_dim * 2, activation=lambda x: tf.nn.sigmoid(x) * 100. + tf.keras.backend.epsilon()))  # two params, 0-10 range
    model.add(tf.keras.layers.Reshape((output_dim, 2)))  # as dimension 2

def custom_top_dirichlet(model, output_dim, schema, batch_size):
    model.add(tf.keras.layers.Dense(output_dim, activation=lambda x: tf.nn.sigmoid(x) * 100. + tf.keras.backend.epsilon()))  # one params per answer, 0-10 range
