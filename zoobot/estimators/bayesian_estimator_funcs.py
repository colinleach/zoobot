...
def get_proxy_mean_squared_error_eval_ops(labels, predictions):
    ...
    observed_vote_fractions = tf.concat([ labels[:, :2]/tf.expand_dims(tf.reduce_sum(labels[:, :2], axis=1), axis=0), labels[:, 2:]/tf.expand_dims(tf.reduce_sum(labels[:, 2:], axis=1), axis=0) ], axis=1)
