...
class SimulatedModel():
    ...
    def delta_acquisition_vs_mean_prediction(self, row):
        entropy_of_mean_prediction = acquisition_utils.binomial_entropy(self.mean_rho_predicton, n_draws=40)  # WARNING will need to be changed for bars N = 10
        row[0].scatter(self.mean_rho_predicton, self.predictive_entropy - entropy_of_mean_prediction)
        row[0].set_ylabel('Delta Predictive Entropy')
        row[1].scatter(self.mean_rho_predicton, self.expected_entropy - entropy_of_mean_prediction)
        row[1].set_ylabel('Delta Expected Entropy')


    def acquisitions_vs_mean_prediction(self, n_acquired, save_dir):
        acquisitions_vs_values(self.model.acquisitions, self.mean_rho_predicton, n_acquired, 'Mean Prediction', save_dir)
    ...
    def calculate_default_metrics(self):
        ...
        self.abs_error = np.abs(self.mean_rho_predicton - self.labels)
        self.square_error = (self.labels - self.mean_rho_predicton) ** 2.
